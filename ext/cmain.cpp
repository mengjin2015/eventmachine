...
/******************
evma_set_alt_certs -- bicdroid
******************/
extern "C" void evma_set_alt_certs (const uintptr_t binding, const char* sni_hostname, const char* privatekey_filename, const char* certchain_filename)
{
	ensure_eventmachine("evma_set_alt_certs");
	EventableDescriptor *ed = dynamic_cast <EventableDescriptor*> (Bindable_t::GetObject (binding));
	if (ed)
		ed->SetAltCerts (sni_hostname, privatekey_filename, certchain_filename);
}
